...
export class ServiceComponent implements OnInit {
  ...
  getServiceDetails() {
    this.route.data.subscribe((data: any) => {
      if (data.services.title === 'Generative Ai Development') {
        this.data1 = this.faqdata.genAiCompany;
      }
      if (data.services.title == 'Ai Consulting') {
        this.data1 = this.faqdata.aiConsulting;
      }
  ...
  prev() {
    if (this.nextServiceCount == 0) {
      ...
      this.nextServiceCount > 0 &&
      this.nextServiceCount <= this.ourAIGenerative.length - 1
    ) {
      this.nextServiceCount--;
      this.router.navigate([
        `${this.ourAIGenerative[this.nextServiceCount].link}`,
      ]);
      this.getServiceDetails();
      this.scrollToTop();
    }
